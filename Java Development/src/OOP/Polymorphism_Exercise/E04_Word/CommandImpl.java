...
public class CommandImpl implements CommandInterface {
    ...
    private String cutTextStorage;

    public CommandImpl(StringBuilder text) {
        this.commandTransforms = new HashMap<>();
        this.text = text;
        setCutTextStorage(cutTextStorage);
    }

    public String getCutTextStorage() {
        return cutTextStorage;
    }

    protected void setCutTextStorage(String cutTextStorage) {
        this.cutTextStorage = cutTextStorage;
    }
    ...
    class PasteTransform implements TextTransform {
        @Override
        public void invokeOn(StringBuilder text, int startIndex, int endIndex) {
            text.replace(startIndex, endIndex, getCutTextStorage());
        }
    }
    @Override
    public void init() {
